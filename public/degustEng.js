...
function runTheCode2()	{
	if(allNotes.length == 0)	{
		var decision = prompt("Are you sure you want to delete the last added field?", "Press the 'OK' button to remove the field");
		if (decision) {
			var parent = document.getElementById("imena");
			parent.removeChild(allDegust[allDegust.length - 1]);
		}
	} else {
		var decision = prompt("Do you want to remove the degustator " + allDegust[allDegust.length - 1].value +  " as well as his ratings?", "Press the 'OK' button to delete the taster and its ratings");
		if (decision) {
			var parent = document.getElementById("imena");
			parent.removeChild(allDegust[allDegust.length - 1]);
...
/////////// Add a field where to put the name of the sample //////////////////
	...
	var wineNumber = document.createElement("INPUT");
		...
		wineNumber.setAttribute("placeholder", "Sample, code");
...
function checkNameSample()	{
	...
	if ((checkProbaName == "") || (checkProbaName.value == checkProbaName.defaultValue))	{
		alert("Please enter a sample name or number before proceeding.");
		return;
	} 
}
					...
					var medal = document.getElementsByClassName("classementNote");
						if(finalNote.innerHTML >= 80) {
							...
							if(finalNote.innerHTML >= 90)	{ 
								medal[referentsia].innerHTML = "PLATINIUM MEDAL <img src='Images/Platinum.jpg' >";
								medal[referentsia].style.padding = "4px"; 
							} else if(finalNote.innerHTML >= 85) {
								medal[referentsia].innerHTML = "GOLD MEDAL <img src='Images/Gold.jpg' >";
							} else if(finalNote.innerHTML >= 82) { 
								medal[referentsia].innerHTML = "SILVER MEDAL <img src='Images/Silver.jpg' >";
							} else {
								medal[referentsia].innerHTML = "BRONZE MEDAL <img src='Images/Bronze.png' >";
							} 
						} else {
...
function runTheCode4()	{
	var decision = prompt("Are you sure you want to delete the last sample?", "Press the 'OK' button to remove the sample");
	if (decision) {
...
function signal()	{
	...
	if(osnovnoSadarzanie.style.display == 'block')	{
		...
		showTheResult.setAttribute("value", "Back to evaluations");

	} else {
		osnovnoSadarzanie.style.display = 'block';
		resultati.style.display = 'none';
		showTheResult.setAttribute("value", "Show the ranking of wines");
	}
}
...
function changeToBg()	{
	...
	if((allDegust.length !== 0) || (allNotes.length !== 0) || (probaKod.length !==0) || (pragNote.value > 0))	{
		var decision = prompt("Are you sure you want to change the language? WARNING! All data will be deleted.", "Press the 'OK' button to change the language");
		if(decision)	{
